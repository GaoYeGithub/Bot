  ...
  async sendWelcomeDM(member) {
    try {
      const embed = new EmbedBuilder()
        ...
        .setDescription(`Hi ${member.user.username}, welcome to our community!\n\n` +
                        'We\'re excited to have you here. Feel free to introduce yourself and get to know everyone.')
        .setColor('#2ecc71')
        .setTimestamp()
        ...
        .addFields(
          { 
            ...
            value: '• Check out the server rules\n• Explore our channels\n• Don\'t hesitate to ask questions', 
            inline: false 
          }
        )
        .setFooter({ 
...
client.on('ready', () => {
  ...
  announcementBot.sendAnnouncementToAll(process.env.GUILD_ID, {
    title: '🌟 Welcome to My Discord Bot! 🌟',
    message: 'A interesting to send and recieve updates, tasks, and announcements like this?\n\n',
    color: '#e91e63',
...
client.on('guildMemberAdd', async (member) => {
  const announcementBot = new AnnouncementBot(client);
  await announcementBot.sendWelcomeDM(member);
});
